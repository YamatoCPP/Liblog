...
namespace ymt {
    struct Log::Impl {
        Impl(const std::filesystem::path& path, MessageImportance defaultImportance) 
            : logFile(path, std::ios::app), defaultImportance(defaultImportance) {}
    ...
    Log::Log(const std::filesystem::path& filePath, MessageImportance defaultImportance) 
        : pimpl{std::make_unique<Impl>(filePath, defaultImportance)} {
    ...
    auto Log::WriteMessage(std::string_view messageText, MessageImportance messageImportance) const -> void {
        if (messageImportance < pimpl->defaultImportance) {
            throw std::invalid_argument{"message importance less than default"};
        }

        const auto strMessageImportance{StringMessageImportance(messageImportance)};
        pimpl->logFile << std::left            << std::setw(30) 
